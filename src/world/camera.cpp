...
const float4x4 cg::world::camera::get_view_matrix() const
{
    ...
    return float4x4(
        { x_axis.x, y_axis.x, z_axis.x, 0 }, { x_axis.y, y_axis.y, z_axis.y, 0 }, { x_axis.z, y_axis.z, z_axis.z, 0 },
...
{
    return float3{ std::sin(theta) * std::cos(phi), std::sin(phi), -std::cos(phi) * std::cos(theta) };
}
