    ...
    for (size_t s = 0; s < shapes.size(); s++)
    {
        ...
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
        {
            ...
            if (shapes[s].mesh.indices[index_offset].normal_index < 0)
            {
                ...
                float3 a{ attrib.vertices[3 * a_id.vertex_index + 0], attrib.vertices[3 * a_id.vertex_index + 1],
                          attrib.vertices[3 * a_id.vertex_index + 2] };
                float3 b{ attrib.vertices[3 * b_id.vertex_index + 0], attrib.vertices[3 * b_id.vertex_index + 1],
                          attrib.vertices[3 * b_id.vertex_index + 2] };
                float3 c{ attrib.vertices[3 * c_id.vertex_index + 0], attrib.vertices[3 * c_id.vertex_index + 1],
...
std::vector<std::shared_ptr<cg::resource<cg::vertex>>> cg::world::model::get_per_shape_buffer() const
{
...
{
    return float4x4({ 1.f, 0.f, 0.f, 0.f }, { 0.f, 1.f, 0.f, 0.f }, { 0.f, 0.f, 1.f, 0.f }, { 0.f, 0.f, 0.f, 1.f });
}
