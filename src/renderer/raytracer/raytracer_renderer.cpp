...
void cg::renderer::ray_tracing_renderer::init()
{
    ...
    for (int x = -1; x <= 1; x++)
        for (int y = -1; y <= 1; y++)
        {
            lights.push_back({ float3{ 0 + x * 0.05f, 1.58f, -0.03f + y * 0.05f }, float3{ 121.f, 58.f, 122.f } / 255.f / 9.f });
        }
}
...
void cg::renderer::ray_tracing_renderer::render()
{
    ...
    raytracer->miss_shader = [](const ray& ray) {
        ...
        payload.color = { 10.f / 255.f * (0.8f - ray.direction.y * 0.8f),
                          12.f / 255.f * (0.8f - ray.direction.y * 0.8f), 23.f / 255.f };
    ...
    raytracer->closest_hit_shader = [&](const ray& ray, payload& payload, const triangle<cg::vertex>& triangle) {
        ...
        for (auto& light : lights)
        {
            ...
            result_color += triangle.diffuse * light.color * std::max(0.f, dot(normal, to_light.direction)) * shadow_factor;
        }
        ...
        if (payload.depth > 0)
        {
            ...
            result_color += 0.3f * float3(ref_result.color.r, ref_result.color.g, ref_result.color.b);
        }
    ...
    shadow_raytracer->closest_hit_shader = [](const ray& ray, payload& payload, const triangle<cg::vertex>& triangle) {
        return payload;
    };
