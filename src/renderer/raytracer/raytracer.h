...
struct ray
{
    ray(float3 position, float3 direction) : position(position) { this->direction = normalize(direction); }
    float3 position;
    float3 direction;
};
...
public:
    ...
    void set_per_shape_vertex_buffer(std::vector<std::shared_ptr<cg::resource<VB>>> in_per_shape_vertex_buffer);
    ...
    std::function<payload(const ray& ray, payload& payload, const triangle<VB>& triangle)> closest_hit_shader = nullptr;
...
template<typename VB, typename RT>
inline void raytracer<VB, RT>::set_per_shape_vertex_buffer(std::vector<std::shared_ptr<cg::resource<VB>>> in_per_shape_vertex_buffer)
{
...
inline void raytracer<VB, RT>::build_acceleration_structure()
{
    for (auto& vertex_buffer : per_shape_vertex_buffer)
    {
        ...
        {
            triangle<VB> triangle(vertex_buffer->item(vertex_id++), vertex_buffer->item(vertex_id++), vertex_buffer->item(vertex_id++));
...
template<typename VB, typename RT>
inline void raytracer<VB, RT>::ray_generation(float3 position, float3 direction, float3 right, float3 up)
{
...
#pragma omp parallel for
        for (int y = 0; y < height; y++)
        {
            ...
            render_target->item(x, y) = RT::from_color(cg::color::from_float3(res_color / SSAA_factor / SSAA_factor));
        }
