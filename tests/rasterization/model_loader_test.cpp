    ...
    GIVEN("An Obj file with 2 triangles")
    {
        ...
        WHEN("Loader load the file")
        {
            ...
            THEN("The loader is able to return correct vertex buffer")
            {
                ...
                for (size_t i = 0; i < expected_vertex_buffer.get_number_of_elements(); i++)
                {
                    REQUIRE(abs(expected_vertex_buffer.item(i).x - vertex_buffer->item(i).x) <= FLT_EPSILON);
                    REQUIRE(abs(expected_vertex_buffer.item(i).y - vertex_buffer->item(i).y) <= FLT_EPSILON);
                    REQUIRE(abs(expected_vertex_buffer.item(i).z - vertex_buffer->item(i).z) <= FLT_EPSILON);
                }
            };
        }
    }
